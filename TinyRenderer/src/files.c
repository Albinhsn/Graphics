...
void initWavefront(struct WavefrontObject *obj) {
  ...
  obj->textureCoordinates =
      (struct Vec3f32 *)malloc(sizeof(struct Vec3f32) * initCap);
  obj->textureCoordinateCapacity = initCap;

  obj->faceCount = 0;
  obj->faceCapacity = initCap;
  obj->faces =
      (struct WavefrontFace *)malloc(sizeof(struct WavefrontFace) * initCap);
}
...
static inline void resizeTextureCoordinates(struct WavefrontObject *obj) {
  if (obj->textureCoordinateCount >= obj->textureCoordinateCapacity) {
    ...
    obj->textureCoordinates =
        realloc(obj->textureCoordinates,
                sizeof(struct Vec3f32) * obj->textureCoordinateCapacity);
  }
}

static void parseWavefrontTexture(struct WavefrontObject *obj,
                                  const char *line) {
  ui8 step;
  ...
  for (i32 i = 0; i < 2; i++) {
    obj->textureCoordinates[obj->textureCoordinateCount - 1].pos[i] =
  ...
  // Parse optional w
  if (curr < strlen(line)) {
    obj->textureCoordinates[obj->textureCoordinateCount - 1].z =
...
static inline void resizeNormals(struct WavefrontObject *obj) {
  if (obj->normalCount >= obj->normalCapacity) {
    ...
    obj->normals =
        realloc(obj->normals, sizeof(struct Vec3f32) * obj->normalCapacity);
  }
}
static void parseWavefrontNormal(struct WavefrontObject *obj,
                                 const char *line) {
  ...
  // Parse x,y,z
  for (i32 i = 0; i < 3; i++) {
    obj->normals[obj->normalCount - 1].pos[i] =
        parseFloatFromString(&line[curr], &step);
    curr += step;
    curr += skipWhitespace(&line[curr]);
  }
...
static inline void resizeVertices(struct WavefrontObject *obj) {
  if (obj->vertexCount >= obj->vertexCapacity) {
    ...
    obj->vertices =
        realloc(obj->vertices, sizeof(struct Vec4f32) * obj->vertexCapacity);
  }
}

static void parseWavefrontVertex(struct WavefrontObject *obj,
                                 const char *line) {
  ui8 step;
  ...
  // Parse x,y,z
  for (i32 i = 0; i < 3; i++) {
    obj->vertices[obj->vertexCount - 1].pos[i] =
        parseFloatFromString(&line[curr], &step);
    curr += step;
    curr += skipWhitespace(&line[curr]);
  }
...
static inline void resizeFaces(struct WavefrontObject *obj) {
  if (obj->faceCount >= obj->faceCapacity) {
    obj->faceCapacity *= 2;
    obj->faces =
        realloc(obj->faces, sizeof(struct Vec4f32) * obj->faceCapacity);
  }
}
...
static void parseWavefrontFace(struct WavefrontObject *obj, const char *line) {
  ...
  face->vertexCount = 0;
  face->verticesData =
      (struct VertexData *)malloc(sizeof(struct VertexData) * vertexCap);
  while (curr < strlen(line)) {
    face->vertexCount++;
    resizeFaceVertexData(face, &vertexCap);

    face->verticesData[face->vertexCount - 1].vertexIdx =
...
static inline void parseWavefrontLine(struct WavefrontObject *obj,
                                      const char *line) {
